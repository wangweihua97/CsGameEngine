...
{
    m_boneLocation = new GLint* [meshes.size()];
    m_shadowBoneLocation = new GLint[ui_BoneArraySize]{ 0 };
    for (unsigned int k = 0; k < pScene->mNumAnimations; k++) {
        string name(pScene->mAnimations[k]->mName.data);
        animations.left.insert(make_pair(name ,k));
    }
    animationIndex = 0;
    animationTime = 0.0f;
    for (int i = 0; i < meshes.size(); i++)
    {
        m_boneLocation[i] = new GLint[ui_BoneArraySize]{0};
        Mesh* m = meshes[i];
        BoneMesh* bm = new BoneMesh(m->vertices, m->indices, m->BaseVertex, Bones);
        delete m;
        bm->setupMesh();
        meshes[i] = bm;
    }
...
{
    if (animations.left.find(animaName) != animations.left.end())
    {
        animationTime = 0.0f;
        animationIndex = animations.left.find(animaName)->second;
    }
}
...
void ModelComponent::LoadModel(string const& path, bool gamma  , bool autoMat)
{
    pScene = NULL;
    _autoMat = autoMat;
...
void ModelComponent::Draw()
{
    ...
    if (_autoMat)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
        {
            Material*  mat = ResourceManager::GetAutoModelMat();
            Window::Instance->m_render.PushRender(0, mat, gameObject->transform->worldTransformMat, glm::mat4(), boost::bind(&ModelComponent::BeforeRenderAnim, this, i));
        }
        return;
    }
    for (int i = 0; i < meshes.size(); i++)
    {
        Material* mat = meshsMat[i];
        if (mat == nullptr)
            mat = ResourceManager::GetDefalutMat();
        Window::Instance->m_render.PushRender(meshes[i], mat, gameObject->transform->worldTransformMat, glm::mat4(), boost::bind(&ModelComponent::BeforeRenderAnim, this, i));
    }
}
...
void ModelComponent::DrawShadow()
{
    for (int i = 0; i < meshes.size(); i++)
    {
        Material* mat = ResourceManager::GetShadowMat();
        Window::Instance->m_render.PushShadowRender(meshes[i], mat, gameObject->transform->worldTransformMat, glm::mat4(), boost::bind(&ModelComponent::BeforeShadowRenderAnim, this, i));
    }
}

void ModelComponent::BeforeRenderAnim(int index)
{
    if (_autoMat)
    {
        meshes[index]->Draw(texturess[index]);
        return;
    }
    if (meshsMat[index] == 0)
        return;
    if (_isPlay)
    {
        meshsMat[index]->GetShader()->SetInteger("_HavaAnimation", 1, false);
        for (unsigned int i = 0; i < boneTransforms.size(); i++) {
            if (m_boneLocation[index][i] == 0 && meshsMat[0] != 0)
            {
                char Name[128];
                memset(Name, 0, sizeof(Name));
                _snprintf_s(Name, sizeof(Name), "gBones[%d]", i);
                m_boneLocation[index][i] = glGetUniformLocation(meshsMat[index]->GetShader()->ID, Name);
            }
            glUniformMatrix4fv(m_boneLocation[index][i], 1, GL_FALSE, &boneTransforms[i][0][0]);
        }
    }
    else
    {
        meshsMat[index]->GetShader()->SetInteger("_HavaAnimation", 0, false);
    }
}

void ModelComponent::BeforeShadowRenderAnim(int index)
{
    Material* shadowMat = ResourceManager::GetShadowMat();
    if (_isPlay)
    {
        shadowMat->GetShader()->SetInteger("_HavaAnimation", 1, false);
        for (unsigned int i = 0; i < boneTransforms.size(); i++) {
            if (m_shadowBoneLocation[i] == 0)
            {
                char Name[128];
                memset(Name, 0, sizeof(Name));
                _snprintf_s(Name, sizeof(Name), "gBones[%d]", i);
                m_shadowBoneLocation[i] = glGetUniformLocation(shadowMat->GetShader()->ID, Name);
            }
            glUniformMatrix4fv(m_shadowBoneLocation[i], 1, GL_FALSE, &boneTransforms[i][0][0]);
        }
    }
    else
    {
        shadowMat->GetShader()->SetInteger("_HavaAnimation", 0, false);
    }
}
...
void ModelComponent::processNode(aiNode* node, glm::mat4 parentTransfrom)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++) {
        ...
        meshsName.push_back(mesh->mName.C_Str());
        meshsMat.push_back(0);
...
Mesh* ModelComponent::processMesh(unsigned int meshId, aiMesh* mesh)
{
    ...
    if (_autoMat)
    {
        ...
        texturess.push_back(textures);
    }
    int offset = baseVertex[meshId];
    // return a mesh object created from the extracted mesh data
    //return BoneMesh(vertices, indices, textures, offset, Bones , m_pShaderProg);
    return new Mesh(vertices, indices, offset);
}
...
void ModelComponent::InitBone()
{
   ...
   /* ResourceManager::GetShaderP("Volume")->Use();
    ...
    /*meshsMat[0]->GetShader()->Use();
    for (unsigned int i = 0; i < ui_BoneArraySize; i++) {

        char Name[128];
        memset(Name, 0, sizeof(Name));
        _snprintf_s(Name, sizeof(Name), "gBones[%d]", i);
        m_boneLocation[i] = glGetUniformLocation(meshsMat[0]->GetShader()->ID, Name);
    }*/
}
...
void ModelComponent_LoadModel(ModelComponent* modelComponent, char* path)
{
    modelComponent->LoadModel(path, false);
}

void ModelComponent_LoadAutoMatModel(ModelComponent* modelComponent, char* path)
{
    modelComponent->LoadModel(path, false ,true);
}

int ModelComponent_GetMeshsCount(ModelComponent* modelComponent)
{
    return modelComponent->meshes.size();
}
const char* ModelComponent_GetMeshName(ModelComponent* modelComponent, int index)
{
    return modelComponent->meshsName[index].c_str();
}

void ModelComponent_SetMeshMat(ModelComponent* modelComponent, int index, Material* mat)
{
    modelComponent->meshsMat[index] = mat;
}

void ModelComponent_InitAnimation(ModelComponent* modelComponent)
{
    modelComponent->InitAnimation();
}
